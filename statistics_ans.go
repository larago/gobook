...
func mode(numbers []float64) (modes []float64) {
	...
	for _, x := range numbers {
		frequencies[x]++
		if frequencies[x] > highsetFrequency {
			highsetFrequency = frequencies[x]
		}
	}
	for x, frequency := range frequencies {
		if frequency == highsetFrequency {
			modes = append(modes, x)
		}
	}
	if highsetFrequency == 1 || len(modes) == len(frequencies) {
		modes = modes[:0]
	}
	sort.Float64s(modes)
	return modes
